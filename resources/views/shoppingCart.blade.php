        ...
        <div class="p-4">
            ...
            <ul class="list-unstyled mb-4">
                <li class="d-flex justify-content-between py-3 border-bottom"><strong class="text-muted">Order
                        Subtotal </strong><strong>{{$sum}} ₺</strong></li>
                <li class="d-flex justify-content-between py-3 border-bottom"><strong class="text-muted">Shipping and
                        handling</strong><strong>{{number_format($sum * 0.2,2)}}₺</strong></li>
                <li class="d-flex justify-content-between py-3 border-bottom"><strong
                        class="text-muted">Tax</strong><strong>{{number_format($sum * 0.18,2)}}₺</strong></li>
                <li class="d-flex justify-content-between py-3 border-bottom"><strong class="text-muted">Total</strong>
                    <h5 class="font-weight-bold">{{number_format($sum + $sum*0.2 + $sum * 0.18,2)}} ₺</h5>
                </li>
            </ul>
            @if($sum == 0)
                <button class="btn btn-danger rounded-pill py-2 btn-block disabled" onclick="">Proceed to checkout
                </button>
            @else
                <button class="btn btn-dark rounded-pill py-2 btn-block" onclick="checkoutBtnAction()">Proceed to
                    checkout
                </button>
            @endif
        </div>
